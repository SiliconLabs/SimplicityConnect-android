...
class Blinky(imageResId: Int, title: String, description: String) : MainMenuItem(imageResId, title, description) {

    override fun onClick(context: Context) {
        if (checkPermission(context)) {
            val connectType = BluetoothService.GattConnectType.BLINKY
            val selectDeviceDialog = SelectDeviceDialog.newDialog(R.string.title_Blinky, R.string.main_menu_description_blinky, null, connectType)
