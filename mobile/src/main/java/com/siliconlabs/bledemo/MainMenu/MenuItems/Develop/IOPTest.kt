...
class IOPTest(imageResId: Int, title: String, description: String) : MainMenuItem(imageResId, title, description) {
    ...
    override fun onClick(context: Context) {
        if (checkPermission(context)) {
            val connectType = BluetoothService.GattConnectType.IOP_TEST
            val selectDeviceDialog = SelectDeviceDialog.newDialog(R.string.title_Interoperability_Test,
                    R.string.main_menu_description_iop_test, null, connectType)
