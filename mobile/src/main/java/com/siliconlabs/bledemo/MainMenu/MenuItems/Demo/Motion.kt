...
class Motion(imageResId: Int, title: String, description: String) : MainMenuItem(imageResId, title, description) {

    override fun onClick(context: Context) {
        if (checkPermission(context)) {
            val connectType = BluetoothService.GattConnectType.MOTION
            val selectDeviceDialog = SelectDeviceDialog.newDialog(R.string.motion_demo_title, R.string
                    .motion_demo_description, null, connectType)
