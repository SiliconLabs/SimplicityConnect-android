...
class Throughput(imageResId: Int, title: String, description: String) : MainMenuItem(imageResId, title, description) {

    override fun onClick(context: Context) {
        if (checkPermission(context)) {
            val connectType = BluetoothService.GattConnectType.THROUGHPUT_TEST
            val selectDeviceDialog = SelectDeviceDialog.newDialog(R.string.title_Throughput, R.string.main_menu_description_throughput, null, connectType)
