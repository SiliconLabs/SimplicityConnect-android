...
class Environment(imageResId: Int, title: String, description: String) : MainMenuItem(imageResId, title, description) {

    override fun onClick(context: Context) {
        if (checkPermission(context)) {
            val connectType = BluetoothService.GattConnectType.ENVIRONMENT
            val selectDeviceDialog = SelectDeviceDialog.newDialog(R.string.environment_demo_title, R.string
                    .environment_demo_description, null, connectType)
