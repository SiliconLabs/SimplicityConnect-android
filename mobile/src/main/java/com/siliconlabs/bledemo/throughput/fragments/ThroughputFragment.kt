...
class ThroughputFragment : Fragment(R.layout.fragment_throughput) {
    ...
    private fun handleClickEvents() {
        btn_start_stop.setOnClickListener {
            ...
            when (viewModel.isUploadActive) {
                true -> {
                    (activity as ThroughputActivity).stopTransmitting()
                    btn_start_stop.text = requireContext().getString(R.string.button_start)
                    toggleRadioButtonsClickability(true)
                }
                false -> {
                    viewModel.isUploadingNotifications = rb_notifications.isChecked
                    (activity as ThroughputActivity).startTransmitting(rb_notifications.isChecked)
                    btn_start_stop.text = requireContext().getString(R.string.button_stop)
                    toggleRadioButtonsClickability(false)
                }
            }
             */
        }
    ...
    private fun toggleRadioButtonsClickability(enabled: Boolean) {
        if (enabled) {
            rb_notifications.isEnabled = true
            rb_indications.isEnabled = true
        }
        else {
            rb_notifications.isEnabled = false
            rb_indications.isEnabled = false
        }
    }

    private fun updateSpeed(speed: Int) {
        speed_view.updateSpeed(getProgress(speed), getSpeedAsString(speed), getUnitAsString(speed), getCurrentMode())
    }

    private fun getCurrentMode() : SpeedView.Mode {
        return when {
    ...
    private fun showDialog(): AlertDialog? {
        return AlertDialog.Builder(context).create().apply {
            ...
            setButton(AlertDialog.BUTTON_NEUTRAL, context.getString(R.string.button_ok)) { _, _ ->  }
            setMessage(context.getString(R.string.throughput_upload_test_message))
            show()
        }
    }
