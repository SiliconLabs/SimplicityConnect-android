...
import com.siliconlabs.bledemo.motion.viewmodels.MotionViewModel
...
class MotionActivity : GdxActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bindBluetoothService(motionGattCallback)
    ...
    private fun setupClickListeners() {
        calibrate.setOnClickListener {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                viewModel.calibrate(bluetoothService?.connectedGatt)
            }, 600)
        }
    }
    ...
    private fun showBrokenSensorsMessage(brokenSensors: Set<ThunderboardSensor>) {
        AlertDialog.Builder(this).apply {
            ...
            setPositiveButton(getString(R.string.button_ok)) { _, _ ->
                bluetoothService?.connectedGatt?.disconnect() ?: onDeviceDisconnect()
            }
            setCancelable(false)
            runOnUiThread { show() }
        }
    ...
    private fun getMotionCharacteristic(characteristic: GattCharacteristic): BluetoothGattCharacteristic? {
        return bluetoothService?.connectedGatt?.getService(GattService.Motion.number)?.getCharacteristic(characteristic.uuid)
    }

    private val motionGattCallback: TimeoutGattCallback = object : TimeoutGattCallback() {
