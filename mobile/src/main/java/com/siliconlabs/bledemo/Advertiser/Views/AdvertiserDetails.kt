...
class AdvertiserDetails(val context: Context) {
    ...
    fun getAdvertiserDetailsView(item: Advertiser, translator: Translator): LinearLayout {
        ...
        return LinearLayout(context).apply {
            ...
            addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_title_advertising_type), advertisingType))

            if (!data.isLegacy) addView(ServiceItemContainerRe(context, context.getString(R.string.Bluetooth_5_Advertising_Extension), getAdvertisingExtensionText(context, translator, item.data)))

            if (data.isAdvertisingData()) {
                if (data.mode.isConnectable()) addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_flags), context.getString(R.string.advertiser_label_flags_default)))

                if (data.advertisingData.includeCompleteLocalName)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_complete_local_name), getCompleteLocalName(context, data.advertisingData)))

                for (manufacturer in data.advertisingData.manufacturers)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_manufacturer_specific_data), getManufacturerData(manufacturer)))

                if (data.advertisingData.includeTxPower)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_tx_power), getTxPower(context, data)))

                if (data.advertisingData.services16Bit.size > 0)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_complete_16bit_service), get16BitServicesText(item.data.advertisingData)))

                if (data.advertisingData.services128Bit.size > 0)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_complete_128bit_service), get128BitServicesText(item.data.advertisingData)))
            }

            if (data.isScanRespData()) {

                if (data.scanResponseData.includeCompleteLocalName)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_complete_local_name), getCompleteLocalName(context, data.scanResponseData)))
                ...
                if (data.scanResponseData.services16Bit.size > 0)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_complete_16bit_service), get16BitServicesText(item.data.scanResponseData)))

                if (data.scanResponseData.services128Bit.size > 0)
                    addView(ServiceItemContainerRe(context, context.getString(R.string.advertiser_data_type_complete_128bit_service), get128BitServicesText(item.data.scanResponseData)))
            }
        }
    }
